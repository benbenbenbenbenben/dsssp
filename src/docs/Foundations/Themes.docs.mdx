...
In the built-in theme, the `theme.filters.colors` array is empty by design. You need to populate it manually, as the number of filters required for your application is not known in advance. Read the [next section](?path=/docs/foundations-colors--docs) to learn how to set the colors.
