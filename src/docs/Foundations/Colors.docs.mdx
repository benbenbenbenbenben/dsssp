import { Meta } from '@storybook/blocks'

<Meta title="Foundations/Colors" parameters={{ viewMode: 'docs' }} />

# Foundations
<br/>
## Colors

When indexes are assigned to filters on the graph, they use colors from the `theme.filters.colors` array. If specific colors are not defined, the filters fallback to `theme.filters.defaultColor`, provided that the components do not have individual color props set.

```tsx
theme.filters.colors = [
  { point: 'red', drag: 'orange', active: 'blue' },
  { point: 'green', drag: 'yellow', active: 'purple' }
]

// ...

<FilterPoint index={0} filter={...} />
<FilterPoint index={1} filter={...} />
```

`drag` and `active` colors for both points and backgrounds may not be assigned. In this case, the component will fallback to the `point` color for all states. [FilterCurve](?path=/docs/components-filtercurve--docs) also uses the `active` color to highlight itself when the flag is set.


#### Structure of the Colors Object

```ts
type FilterThemeColors = {
  // point colors for each state
  point?: CSSProperties['color']
  drag?: CSSProperties['color']
  active?: CSSProperties['color']
  // point background colors for each state
  background?: CSSProperties['color']
  dragBackground?: CSSProperties['color']
  activeBackground?: CSSProperties['color']
  // gradient stop color
  // gradient's start color is always transparent, unless `fill` prop is set to true
  gradient?: CSSProperties['color']
  // curve color
  curve?: CSSProperties['color']
}
```
